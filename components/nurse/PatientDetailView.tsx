...
interface NursePatientDetailViewProps {
    patient: Patient;
    onBack: () => void;
}

const NursePatientDetailView: React.FC<NursePatientDetailViewProps> = ({ patient, onBack }) => {
            ...
            <Card title="Prescrições Ativas">
                {prescriptions.length > 0 ? (
                    <ul className="space-y-3 max-h-60 overflow-y-auto">
                        {prescriptions.filter(p => p.status !== PrescriptionStatus.Cancelled).map(p => (
                    ...
                    <p className="text-center text-gray-500 dark:text-gray-400 py-4">Nenhuma prescrição ativa.</p>
                )}
            </Card>

            <Card title="Histórico de Administração Recente">
                 <ul className="space-y-3 max-h-60 overflow-y-auto">
...
export default NursePatientDetailView;
